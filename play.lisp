...
(defun play-fun (clip)
  ...
  (let ((tape (tape clip))
    ...
    (declare (moment elapsed)
	     (fixnum pos-a pos-b)
	     (single-float gain-b))
    (lambda (dst index time start gain)
      ...
      (pos-play dst index tape pos-a (- gain gain-b))
      (pos-play dst index tape pos-b gain-b)
      modulus)))
...
(defun cycle (fun)
  ...
  (let ((modulus 0)
    ...
    (lambda (dst index time start gain)
      ...
      (cond
	(first-run
	 (setf modulus (funcall fun dst index time start gain) ;; set start times
	...
	((moment< time this-start)
	 (shiftf (frame next-start) ;; shift start-times backwards
      ...
      (funcall fun dst index time prev-start gain)
      (funcall fun dst index time this-start gain)
      (funcall fun dst index time next-start gain))))
    ...
    (lambda (dst index time start gain)
	 ...
	 for fun in play-funs do
	   (incf (frame sub-start)
		 (the fixnum
		      (funcall (the function fun) dst index time sub-start gain)))
	 finally
...
(defun gain (db fun)
  ...
  (let ((new-gain (db-gain db)))
    ...
    (lambda (dst index time start old-gain)
      (declare (fixnum index)
	       (moment time start)
	       (single-float old-gain)
	       (optimize (speed 3) (space 0) (safety 0)
			 (debug 0) (compilation-speed 0)))
      (funcall fun dst index time start (* new-gain old-gain)))))
...
(defun mute (fun)
  ...
  (lambda (dst index time start gain)
    (declare (fixnum index)
	     ...
	     (optimize (speed 3) (space 0) (safety 0)
		       (debug 0) (compilation-speed 0)))
    (funcall fun dst index time start 0.0)))
